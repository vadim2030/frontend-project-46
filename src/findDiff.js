...
const findDiff = (path1, path2) => {
  ...
  const newObj = keys.map((key) => {
    ...
    if (!_.has(path1, key)) {
      return { key, type: 'added', value: data2 };
    }
    if (!_.has(path2, key)) {
      return { key, type: 'deleted', value: data1 };
    }
    if (_.isPlainObject(data1) && _.isPlainObject(data2)) {
      return { key, type: 'nested', children: findDiff(data1, data2) };
    }
    ...
    return { key, type: 'unchanged', value: data1 };
  });
